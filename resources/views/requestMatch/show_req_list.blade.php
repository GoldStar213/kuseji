                               ...
                               <tbody>
                                    @foreach($request_matches as $request_match)
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <div class="d-flex align-items-center">
                                                    ...
                                                    <div class="d-flex flex-column justify-content-center">
                                                        ...
                                                        <div class="fw-semibold fs-6 text-gray-400">{{ $requestItem->description }}</div>
                                                    </div>
                                                    <!--end::Info-->
                                                </div>
                                                <!--end::User-->
                                            ...
                                            <td>
                                                <select name="receive_grade" id="receive_grade_{{ $i }}" class="form-select" data-control="select2" data-placeholder="Select an option">
                                                    <option value="0" {{ $request_match->receive_grade != null ? 'selected': ''}}>0(拒否)</option>
                                                    <option value="1" {{ $request_match->receive_grade != null ? 'selected': ''}} >1</option>
                                                    <option value="2" {{ $request_match->receive_grade != null ? 'selected': ''}} >2</option>
                                                    <option value="3" {{ $request_match->receive_grade != null ? 'selected': ''}} >3</option>
                                                    <option value="4" {{ $request_match->receive_grade != null ? 'selected': ''}} >4</option>
                                                    <option value="5" {{ $request_match->receive_grade != null ? 'selected': ''}} >5(同意)</option>
                                                </select>
                                            </td>
                                            <td>
                                                <button class="btn btn-primary {{ $request_match->receive_grade != null ? 'disabled': ''}}"  onclick="reply_request({{ $myItem->id }}, {{ $requestItem->id }}, {{ $i }})">確 認</button>
                                            </td>
                                        </tr>
                                    @endforeach
                               </tbody>
